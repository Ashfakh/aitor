...
class OpenAILLM(BaseLLM):
    ...
    async def generate(
        ...
        **kwargs
        ...
        try:
            ...
            for key in ["top_p", "n", "stop", "presence_penalty", "frequency_penalty", "logit_bias", "user"]:
                if key in kwargs:
    ...
    async def generate_json(
        ...
        **kwargs
        ...
        if "gpt-4" in self.model and "1106" in self.model or "gpt-4-turbo" in self.model:
            kwargs["response_format"] = {"type": "json_object"}
            
            # Add instruction to system message
            json_messages = messages.copy()
            json_messages.insert(0, Message(
                role="system",
...
class LLMFactory:
    ...
    @classmethod
    def create(cls, provider: Union[str, LLMProvider], config: Dict[str, Any]) -> BaseLLM:
        """
        Create LLM instance.
