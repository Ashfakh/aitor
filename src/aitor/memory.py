...
class ReactMemory:
    ...
    def add_message(self, role: Union[MessageRole, str], content: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        with self._lock:
            ...
            for step in steps:
                ...
                if step.tool_name and step.tool_result:
                    result_summary = "SUCCESS" if step.tool_result.success else f"ERROR: {step.tool_result.error}"
                    context_lines.append(f"  TOOL_RESULT: {step.tool_name} -> {result_summary}")
